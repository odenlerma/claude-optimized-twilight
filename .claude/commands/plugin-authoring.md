...
Load `.claude/rules/plugin-authoring.md` and apply its six rules to all subsequent work under `plugins/` in this session.
...
4. From this point on, when creating or editing any file under `plugins/`, conform to all six rules. If a request would violate a rule, surface the conflict before acting.
