...
Confirm every authored skill is visible to `npx skills` (skills.sh distribution). See `${CLAUDE_PLUGIN_ROOT}/rules/skills-distribution.md`.
...
Load `${CLAUDE_PLUGIN_ROOT}/rules/plugin-authoring.md` once. For each plugin in scope, walk every file under `plugins/<name>/{skills,commands,agents,hooks,mcp}/` and check:
