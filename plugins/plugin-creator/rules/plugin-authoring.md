...
Every skill needs valid `SKILL.md` frontmatter (`name` + `description`) and a CLI-discoverable path, and plugin skills must be grouped in root `skills.sh.json`. See [`skills-distribution.md`](skills-distribution.md). Verify with `npx skills add . --list`.
